...
const Profile = () => {
  ...
  const onSubmitHandler = async (_id: any, e: any) => {
    e.preventDefault();
    const id = _id;
    console.log(id);
    try {
  ...
  const meDataFetch = async () => {
    ...
    console.log(data);
    setDataUser(data);
    getImageById(data._id);
  };

  const getImageById = async (id: any) => {
    try {
      const response = await axios.get(`/api/users/imageUpload/${id}`);
      console.log(response.data.images);
      const imageData = {
        image_url: response.data.images.image_url,
        public_id: response.data.images.public_id,
        _id: response.data.images._id,
      };
      setImages(imageData);
    } catch (error: any) {
      ...
      <div className="mx-auto max-w-242.5">
        ...
        <div className="overflow-hidden rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
          <div className="relative z-20 h-35 md:h-65">
            <Image
              key={images._id}
              src={images.image_url || "/images/cover/cover-01.png"}
              alt="profile cover"
              className="h-full w-full rounded-tl-sm rounded-tr-sm object-cover object-center"
              style={{ objectPosition: "top" }} // Add this style to position the image at the top
              layout="fill"
              objectFit="cover"
            />
          ...
          <div className="px-4 pb-6 text-center lg:pb-8 xl:pb-11.5">
            <form onSubmit={(e) => onSubmitHandler(dataUser._id, e)}>
            ...
            <div className="mt-4">
              ...
              <p className="font-medium"> {dataUser.role}</p>
